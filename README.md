...
- **[Local stdio](#local-stdio)** is the simplest starting point for one local MCP client. The
  client launches the server without opening a network listener.
- **[Direct Node.js — Streamable HTTP](#direct-nodejs--streamable-http)** runs a persistent
  service for HTTP-capable MCP clients, listening on loopback by default.
- **[Docker Compose — Streamable HTTP](#docker-compose)** runs the published container when you
  prefer to manage the service with Docker, with loopback-only host publishing by default.
...
For a personal example of how I organize my SimpleLogin aliases, read
[How I organize my email](https://www.antoinemenard.com/articles/how-i-organize-my-email/).
...
pnpm commands do not automatically load `.env`; the [Direct Node.js example](#direct-nodejs--streamable-http)
