...
- **Docker:** the bundled `docker-compose.yml` binds the container to `0.0.0.0` (required
  for Docker port forwarding) but publishes only to the host's loopback
  (`127.0.0.1:3000:3000`), so it is host-local by default. To reach it from the LAN, set
  `MCP_AUTH_TOKEN` and widen the `ports` mapping.
