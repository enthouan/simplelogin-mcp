...
1. **Identify the run shape.** Record Local stdio, Direct Node.js — Streamable HTTP, or Docker
   Compose — Streamable HTTP and the simplelogin-mcp version or commit.
...
4. **Verify credentials with a read-only call.** After the client discovers tools, the expected
   tool is `account_get_stats`. Confirm that it is marked read-only before approving it. Its
   aggregate counts still pass through the client and its configured model.
...
## Local stdio connection problems
